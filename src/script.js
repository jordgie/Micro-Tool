document.addEventListener('DOMContentLoaded', function () {
    ...
    function showFlexboxGenerator() {
        screen.innerHTML = `
            <div id="flexboxGenerator">
                <h2>Flexbox Generator</h2>
    
                <div class="flexbox-input-row">
                    <label for="childCount">Child Count:</label>
                    <input type="number" id="childCount" placeholder="Enter child count">
                </div>
                ...
                <button id="generateFlexbox">Generate CSS</button>
                <button id="copyFlexbox">Copy CSS</button>
                <pre id="flexboxResult"></pre>
                <div id="flexPreview" class="flex-preview"></div>
            </div>
        `;
        ...
        generateFlexbox.addEventListener('click', () => {
            const childCount = document.getElementById('childCount').value || 3;
            ...
            // Generate accurate child elements for preview
            const childElements = Array.from({ length: childCount }, (_, index) => `<div class="box">${index + 1}</div>`).join('');
            flexPreview.innerHTML = childElements;
            flexPreview.style.cssText = cssCode;
        });
    
        copyFlexbox.addEventListener('click', () => {
            const range = document.createRange();
            range.selectNode(document.getElementById('flexboxResult'));
            ...
            try {
                ...
                console.error('Unable to copy CSS code', err);
            }
        });
    }
